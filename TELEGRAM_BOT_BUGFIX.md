...
## Дополнительная проблема: Старые пользователи без `updated_at`
...
### Причина
Пользователи, созданные до обновления, не имели поля `updated_at` в БД.
...
```bash
# Добавление updated_at для пользователей без него
mongosh test_database --eval '
  db.user_settings.updateMany(
    { updated_at: { $exists: false } },
    { $set: { updated_at: new Date() } }
  )
'

# Добавление id (UUID) для пользователей без него
mongosh test_database --eval '
  db.user_settings.find({ id: { $exists: false } }).forEach(user => {
    db.user_settings.updateOne(
      { _id: user._id },
      { $set: { id: require("crypto").randomUUID() } }
    )
  })
'
```
...
Файл: `/app/backend/migrate_users.py`

Скрипт автоматически:
- ✅ Добавляет поле `id` (UUID) для пользователей без него
- ✅ Добавляет поле `updated_at` для пользователей без него
- ✅ Добавляет поле `created_at` на основе ObjectId timestamp
- ✅ Показывает статистику пользователей

Запуск:
```bash
cd /app/backend
/root/.venv/bin/python migrate_users.py
```

Результат миграции:
```
✅ Все пользователи уже имеют поле 'id'
✅ Все пользователи уже имеют поле 'updated_at'
✅ Все пользователи уже имеют поле 'created_at'

📊 Статистика пользователей:
   Всего пользователей: 3
   С выбранной группой: 0
   Без группы: 3
...
# Запуск миграции пользователей (при необходимости)
cd /app/backend && /root/.venv/bin/python migrate_users.py
```
...
✅ Все существующие пользователи мигрированы
✅ Новые пользователи создаются с полным набором полей
