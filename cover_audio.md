...
- [x] Создать `/app/backend/cover_service.py` ✅ В РАБОТЕ
- [ ] Реализовать класс `CoverService`
- [ ] Добавить методы кеширования

### Этап 2: Backend - Интеграция (15 мин)
- [ ] Модифицировать `music_service.py` - добавить `enrich_tracks_with_covers`
- [ ] Модифицировать `server.py` - инициализация сервиса
- [ ] Обновить эндпоинты `/api/music/*`

### Этап 3: Индексы MongoDB (5 мин)
- [ ] Создать индексы для коллекции `cover_cache`

### Этап 4: Тестирование (10 мин)
- [ ] Тест API `/api/music/my` - проверить наличие обложек
- [ ] Тест кеширования - повторный запрос должен быть из кеша
