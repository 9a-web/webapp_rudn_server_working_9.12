...
| Backend LOC | **~25,000** |
| Frontend компонентов | **105** (72 основных + 15 journal + 12 music + 6 other) |
| API endpoints | **200** |
...
```
/app/
├── backend/                    # FastAPI backend (25,000 LOC)
│   ├── server.py              # Главный сервер (12,753 LOC, 200 endpoints)
│   ├── models.py              # Pydantic схемы (2,262 LOC)
│   ├── telegram_bot.py        # Telegram Bot (1,347 LOC)
│   ├── scheduler_v2.py        # Уведомления V2 (828 LOC)
│   ├── achievements.py        # Достижения (770 LOC)
│   ├── lk_parser.py           # Парсер ЛК РУДН (380 LOC)
│   ├── scheduler.py           # Старый планировщик (383 LOC)
│   ├── vk_auth_service.py     # VK авторизация (350 LOC)
│   ├── music_service.py       # VK Music сервис (333 LOC)
│   ├── rudn_parser.py         # Парсер API РУДН (311 LOC)
│   ├── cover_service.py       # Обложки треков (270 LOC)
│   ├── notifications.py       # Telegram уведомления (194 LOC)
│   ├── weather.py             # OpenWeatherMap API
│   ├── config.py              # Конфигурация ENV
│   ├── cache.py               # Кэширование данных
│   └── requirements.txt       # Python зависимости
│
├── frontend/                   # React 19 frontend (~46,000 LOC)
│   └── src/
│       ├── App.jsx            # Главный компонент (1,610 LOC)
│       ├── components/        # 72 основных компонента
│       │   ├── TasksSection.jsx      # (2,464 LOC) - Задачи
│       │   ├── ProfileModal.jsx      # (1,542 LOC) - Профиль
│       │   ├── AdminPanel.jsx        # (974 LOC) - Админка
...
## 🔌 API ENDPOINTS (200 всего)

### Группировка по модулям:

| Модуль | Endpoints | Описание |
|--------|-----------|----------|
| Расписание РУДН | 6 | Факультеты, фильтры, расписание |
| Пользователи | 15 | Настройки, тема, история |
| Планировщик | 5 | Синхронизация, события |
| Личные задачи | 9 | CRUD, подзадачи, продуктивность |
| Комнаты | 14 | CRUD, приглашения, активность |
| Групповые задачи | 16 | CRUD, подзадачи, комментарии |
| Журнал посещений | **26** | Журналы, студенты, предметы, занятия |
| VK Music | **25** | Поиск, стрим, плейлисты, комнаты |
| Друзья | 15 | Запросы, блокировки, поиск |
| In-App уведомления | 8 | CRUD, настройки |
| Достижения | 5 | Список, трекинг |
| Реферальная система | 4 | Коды, статистика, дерево |
| ЛК РУДН | 4 | Подключение, данные |
| Админ панель | **14** | Статистика, пользователи, **онлайн мониторинг** |
| Web Sessions | **11** | QR-авторизация, устройства |
| Приватность | 2 | GET/PUT настроек |
| Бэкапы | 3 | Экспорт БД |
| Прочее | 5 | Погода, YouTube, VK Video |
