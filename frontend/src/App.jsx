...
const Home = () => {
  ...
  //
  // ⚠️ ВАЖНО: backend хранит user_settings.telegram_id = 10^10 + uid (pseudo_tid)
  // для не-Telegram пользователей (VK/Email/QR), см. auth_utils.pseudo_tid_from_uid.
  // Поэтому тут при отсутствии реального telegram_id тоже формируем pseudo_tid,
  // иначе GET /api/user-settings/{id} возвращает 404 и мы показываем WelcomeScreen
  // даже для уже зарегистрированных пользователей.
  const PSEUDO_TID_OFFSET = 10_000_000_000; // sync с auth_utils.py
  const user = React.useMemo(() => {
    if (authUser) {
      let telegramId = authUser.telegram_id || null;
      if (!telegramId && authUser.uid) {
        const uidInt = parseInt(String(authUser.uid), 10);
        if (Number.isFinite(uidInt)) {
          telegramId = PSEUDO_TID_OFFSET + uidInt;
        }
      }
      if (telegramId) {
        return {
          ...
          photo_url: authUser.photo_url || rawTgUser?.photo_url || null,
          uid: authUser.uid,
          is_linked: true,
          is_guest: false,
        };
  ...
  // 🛠 Dev Console — привязка React state
  useEffect(() => {
    if (window.dev && window.dev._bind) {
      window.dev._bind({
  ...
  const loadUserData = useCallback(async () => {
    ...
    try {
      ...
      if (currentUser.is_guest) {
        ...
        // Данных нет — новый пользователь, сразу показываем выбор группы
        console.log('👤 Новый гостевой пользователь — показываем GroupSelector');
      ...
      if (settings && settings.group_id && settings.facultet_id) {
        ...
        if (isLinkedUser && cachedSettings && cachedSettings.group_id && cachedSettings.facultet_id) {
          ...
          // Связанный пользователь без настроек — сразу показываем выбор группы
          console.log('Linked user without settings - showing group selector');
          setShowGroupSelector(true);
        } else {
          // Гостевой пользователь — сразу показываем выбор группы
          setShowGroupSelector(true);
      ...
      if (err.message === 'Пользователь не найден') {
        // Сразу показываем выбор группы для всех новых пользователей
        console.log('User not found - showing group selector');
        setShowGroupSelector(true);
      } else {
  ...
  // WelcomeScreen больше не показывается — новые пользователи идут сразу в GroupSelector.
  // Старый блок `if (showWelcomeScreen) { ... <WelcomeScreen /> ... }` удалён.
