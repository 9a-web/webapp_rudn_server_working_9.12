...
// ───────────────────────────────────────────
// FreeWindowBlock — свободное окно
// ───────────────────────────────────────────
const FreeWindowBlock = ({ window: freeWindow, index }) => {
  const startMin = parseTime(freeWindow.start);
  const endMin = parseTime(freeWindow.end);
  
  if (startMin === null || endMin === null) return null;
  
  const top = minToPx(startMin);
  const height = durationToPx(endMin - startMin);
  const duration = freeWindow.duration_minutes;
  ...
  return (
    ...
    >
      ...
      {/* Border */}
      <div 
        className="absolute inset-0 rounded-xl border border-dashed border-emerald-300"
      />
    </motion.div>
  );
};
