...
export const SharedScheduleView = ({ telegramId, selectedDate, weekNumber = 1, onClose, hapticFeedback, onFriendPickerChange, onShareModalChange }) => {
  ...
  return (
    ...
    >
      ...
      {/* ─── Скрытый контейнер для генерации изображения ─── */}
      <div
        ref={exportRef}
        style={{
          display: 'none',
      ...
      >
        <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '20px' }}>
          ...
          <div style={{ marginBottom: '16px', textAlign: 'center' }}>
            <div style={{ fontSize: '18px', fontWeight: '700', color: '#1c1c1c', marginBottom: '4px' }}>
              📅 Совместное расписание
            </div>
            <div style={{ fontSize: '13px', color: '#888', marginBottom: '8px' }}>
              {selectedDay} • Неделя {weekNumber}
            </div>
            {/* Участники */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
              {activeParticipantIds.map((pId) => {
                ...
                if (!participant) return null;
                return (
                  <div key={pId} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 10px', borderRadius: '12px', backgroundColor: participant.color + '18' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: participant.color }} />
                    <span style={{ fontSize: '11px', fontWeight: '600', color: participant.color }}>
                      {participant.telegram_id === telegramId ? 'Вы' : participant.first_name}
                    </span>
                  </div>
                );
          ...
          <div style={{ position: 'relative', backgroundColor: '#fafafa', borderRadius: '16px', border: '1px solid #f0f0f0', padding: '0' }}>
            <div style={{ position: 'relative', height: `${displayHeight}px` }}>
              ...
              {hourLines.map(({ hour, label, top, isMain }) => (
                <div key={`exp-h-${hour}`} style={{ position: 'absolute', width: '100%', top: `${top}px` }}>
                  <div style={{
                    ...
                    color: isMain ? '#999' : '#ccc', width: `${TIME_LABEL_WIDTH - 10}px`, textAlign: 'right',
                  }}>
                    {label}
                  </div>
                  <div style={{
                    position: 'absolute', left: `${TIME_LABEL_WIDTH}px`, right: '4px', height: '1px',
                    backgroundColor: isMain ? '#e8e8e8' : '#f2f2f2',
                  }} />
                </div>
              ))}
              ...
              {/* Контейнер событий */}
              <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${TIME_LABEL_WIDTH + 4}px`, right: '4px' }}>
                {/* Свободные окна */}
                {dayFreeWindows.map((fw, idx) => {
                  const fwStartMin = parseTime(fw.start);
                  const fwEndMin = parseTime(fw.end);
                  ...
                  const hours = Math.floor(duration / 60);
                  const mins = duration % 60;
                  const durationText = hours > 0 ? `${hours}ч${mins > 0 ? ` ${mins}м` : ''}` : `${mins}м`;
                  return (
                    <div key={`exp-fw-${idx}`} style={{
                      position: 'absolute', left: 0, right: 0, top: `${fwTop}px`, height: `${Math.max(fwHeight, 24)}px`,
                      borderRadius: '12px', border: '1.5px dashed #6ee7b7', backgroundColor: '#ecfdf520',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      <span style={{ fontSize: '11px', fontWeight: '500', color: '#059669' }}>
                        ☕ Свободны {durationText} ({fw.start} – {fw.end})
                      </span>
                    </div>
                  );
                })}
                ...
                {activeParticipantIds.map((pId, colIdx) => {
                  ...
                  if (!participant) return null;
                  const colWidthPct = 100 / totalColumns;
                  const leftPct = colWidthPct * colIdx;
                  return events.map((event, eIdx) => {
                    ...
                    return (
                      <div key={`exp-ev-${pId}-${eIdx}`} style={{
                        position: 'absolute', top: `${evTop}px`, minHeight: `${evH}px`,
                        left: `calc(${leftPct}% + 2px)`, width: `calc(${colWidthPct}% - 4px)`,
                        backgroundColor: participant.color + '14', borderLeft: `3.5px solid ${participant.color}`,
                        borderRadius: '12px', padding: '6px 8px', boxSizing: 'border-box',
                      }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
                          <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: participant.color, flexShrink: 0 }} />
                          <span style={{ fontSize: '9px', fontWeight: '500', color: participant.color }}>
                            {participant.telegram_id === telegramId ? 'Вы' : participant.first_name}
                          </span>
                        </div>
                        <div style={{ fontSize: '11px', fontWeight: '600', color: '#1c1c1c', lineHeight: '1.3', marginBottom: '2px', wordBreak: 'break-word' }}>
                          {event.discipline}
                        </div>
                        <div style={{ fontSize: '9px', color: '#999' }}>
                          {sStr} – {eStr}{event.auditory ? ` · ${event.auditory}` : ''}
                        </div>
                      </div>
                    );
                  });
          ...
          {/* Футер */}
          <div style={{ marginTop: '12px', textAlign: 'center', fontSize: '10px', color: '#bbb' }}>
            RUDN GO • Совместное расписание
          </div>
        </div>
      </div>
