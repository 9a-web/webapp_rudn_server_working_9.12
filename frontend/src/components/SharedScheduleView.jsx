...
import { Users, Plus, X, Clock, UserPlus, Coffee, Trash2, AlertCircle, Share2, Copy, Check, Download, Image, Send, Smartphone, ChevronDown, ChevronRight } from 'lucide-react';
...
const MIN_COLUMN_WIDTH = 320; // минимальная ширина колонки — широкие колонки = текст помещается горизонтально, не вылезает за время
...
export const SharedScheduleView = ({ telegramId, selectedDate, weekNumber = 1, onClose, hapticFeedback, onFriendPickerChange, onShareModalChange, externalShareTrigger }) => {
  ...
  const generateCanvas = useCallback(async () => {
    try {
      ...
      const toY = (min) => tlPadTop + (min - offsetMin) * canvasPxPerMin;
  ...
  const handleSendToBot = useCallback(async () => {
    ...
    try {
      ...
      const dateStr = selectedDate
        ...
        : '';
      const caption = `<tg-emoji emoji-id="5307850030416156311">📅</tg-emoji> <b>Совместное расписание</b>\n<tg-emoji emoji-id="5312474198365462799">📆</tg-emoji> ${selectedDay ? selectedDay + ', ' : ''}${dateStr}`;
  ...
  const displayHeight = timelineHeight + 20; // полный диапазон 0:00–23:00
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative" style={{ height: `${displayHeight}px`, minHeight: '200px', minWidth: `${Math.max(420, totalColumns * MIN_COLUMN_WIDTH + TIME_LABEL_WIDTH + 10)}px` }}>
          ...
          >
            ...
            {dayFreeWindows.map((fw, idx) => {
              ...
              let fwEndMin = parseTime(fw.end);
              if (fw.end === '24:00') fwEndMin = 1440;
