...
import { ChevronLeft, Trophy, Settings, QrCode, X, Sliders, Smartphone, Users, Link2, Snowflake, Trash2, AlertTriangle, GraduationCap, Pen, ShieldCheck, Copy, Award, ChevronRight, Star, Lock, KeyRound, LogOut } from 'lucide-react';
...
const ProfileScreen = ({ isOpen, onClose, user, userSettings, profilePhoto, hapticFeedback, onThemeChange, initialTab }) => {
  ...
  const settingsItems = [
    ...
    }] : []),
    {
      ...
      sublabel: 'Завершить текущую сессию',
      color: '#F59E0B',
      action: () => { closeSheet(); setTimeout(() => setShowLogoutConfirm(true), 200); },
    },
    {
      id: 'delete',
      icon: Trash2,
      ...
      <AnimatePresence>
        {showLogoutConfirm && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            style={{ backgroundColor: 'rgba(0,0,0,0.85)' }}
            onClick={() => !logoutLoading && setShowLogoutConfirm(false)}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <LogOut style={{ width: '40px', height: '40px', color: '#F59E0B', margin: '0 auto 12px' }} />
              <div style={{
                fontFamily: "'Poppins', sans-serif",
                fontWeight: 600,
                fontSize: '16px',
              ...
              <div style={{
                ...
                Вам потребуется войти заново, чтобы продолжить пользоваться приложением
              </div>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      // 1) инвалидируем JWT на сервере (если возможно) + чистим state
                      await logout();
                      // 2) полная очистка legacy-ключей (tgId, guest_id, кэши и пр.)
                      clearAllLocalAuthData();
                      if (hapticFeedback) hapticFeedback('notification', 'success');
                      setShowLogoutConfirm(false);
                      onClose();
                      // 3) переход на экран входа — AuthGate без JWT направит туда
                      window.location.replace('/login');
                    } catch (error) {
                      console.error('Ошибка выхода:', error);
                      if (hapticFeedback) hapticFeedback('notification', 'error');
                      // Даже если запрос упал — чистим локально и уходим на /login
                      clearAllLocalAuthData();
                      window.location.replace('/login');
                    } finally {
