...
export const ImportLectureModal = ({ isOpen, onClose, onCreated }) => {
  ...
  useEffect(() => {
    if (!isOpen) {
      // сброс при закрытии
      setText(''); setTitle(''); setNumQuestions(10);
      setError(null); setLoading(false); setDragging(false);
    }
  }, [isOpen]);
  ...
  const readFile = useCallback((file) => {
    ...
    if (!isText) {
      setError('Поддерживаются только текстовые файлы (.txt, .md)');
      return;
    }
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-white/[0.06]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-foreground font-semibold text-[15px] leading-tight">Новый тест из лекции</h2>
            ...
            {loading ? (
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                >
                  ...
                  <p className="text-muted-foreground text-[11px] mt-1">Текстовый файл .txt или .md</p>
                </div>

                {/* Title */}
                <div>
                  <label className="block text-[12px] text-muted-foreground mb-1.5">Название теста</label>
                  <input
                    type="text"
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    placeholder="Например: Лекция 5 — Термодинамика"
                    maxLength={120}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[12px] text-muted-foreground">Количество вопросов</label>
