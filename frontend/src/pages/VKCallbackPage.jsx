...
const VKCallbackPage = () => {
  ...
  return (
    <AuthLayout
      title="VK ID"
      subtitle={status === 'error' ? 'Произошла ошибка' : 'Обработка входа'}
    >
      {status === 'processing' && (
        <div className="flex flex-col items-center gap-4 py-8 text-center">
          <Loader2 className="h-10 w-10 animate-spin text-indigo-400" />
          <div className="text-sm text-white/70">Обмениваем code на токен...</div>
        </div>
      )}
      {status === 'success' && (
        <div className="flex flex-col items-center gap-4 py-6 text-center text-sm text-white/70">
